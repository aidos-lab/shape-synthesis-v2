...
def train(args):
    ...
    # data_loader_pre = DataLoader(
    #     im_dataset, batch_size=train_config["autoencoder_batch_size"], shuffle=True
    # )
    ...
    data_loader_pre, _ = get_dataloaders(config, dev=False)
    transform = get_transform(compiled=False, resolution=32, d=2)
    ...
    lpips_model = torch.compile(LPIPS().eval().to(device))
    ...
    for epoch_idx in range(num_epochs):
        ...
        for pc, _ in tqdm(data_loader):
            pc = pc.cuda()
            index = torch.arange(0, len(pc), device="cuda").repeat_interleave(
                repeats=256
            )
            im = transform(pc.view(-1, 2), index).unsqueeze(1)
            step_count += 1
            ...
            codebook_losses.append(
                train_config["codebook_weight"]
                * quantize_losses["codebook_loss"].item()
            )
            ...
            lpips_loss = torch.mean(lpips_model(output.cuda(), im.to(device)))
