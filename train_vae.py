...
train_dataset = datasets.MNIST(
    "./data/raw/mnist", train=True, download=True, transform=transform
)
test_dataset = datasets.MNIST("./data/raw/mnist", train=False, transform=transform)
...
def train(epoch):
    ...
    for batch_idx, (data, _) in enumerate(train_loader):
        ...
        if batch_idx % args.log_interval == 0:
            print(
                "Train Epoch: {} [{}/{} ({:.0f}%)]\tMSE: {:.6f}\tKL: {:.6f}\tlog_sigma: {:f}".format(
                    epoch,
                    batch_idx * len(data),
    ...
    with torch.no_grad():
        for i, (data, _) in enumerate(tqdm(test_loader)):
            ...
            if i == 0:
                ...
                save_image(
                    comparison.cpu(),
                    "vae_logs/{}/reconstruction_{}.png".format(
                        args.log_dir, str(epoch)
                    ),
                    nrow=n,
                )
